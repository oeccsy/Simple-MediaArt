...
def free_drawing(canvas_width=640, canvas_height=480, init_brush_radius=3):
    ...
    while True:
        ...
        if mouse_left_button_click:
           cv.circle(draw, mouse_xy, brush_radius, palette[brush_color], -1)
           # send position via UDP
           sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
           sock.sendto(struct.pack('ii', mouse_xy[0], mouse_xy[1]), (UDP_IP, UDP_PORT))
        
        # Show the canvas
        draw_copy = draw.copy()
        info = f'Brush Radius: {brush_radius}'
        cv.putText(draw_copy, info, (10, 25), cv.FONT_HERSHEY_DUPLEX, 0.6, (127, 127, 127), thickness=2)
        cv.putText(draw_copy, info, (10, 25), cv.FONT_HERSHEY_DUPLEX, 0.6, palette[brush_color])
        cv.imshow('Free Drawing', draw_copy)
...
if __name__ == '__main__':
    img = cv.imread('./OpenCV/Resources/myCarrot.png')
    draw = img.copy()
    free_drawing()